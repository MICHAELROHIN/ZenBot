  ...
  Future<void> sendMessage(String message) async {
    ...
    try {
      final response = await http
          .post(
        Uri.parse('http://10.0.2.2:5000/chat'), // Emulator; change to IP for device
      ...
      print("API exception: $e"); // Log full exception
      setState(() {
        messages.add({
          "sender": "bot",
...
Widget _buildQuickReplies() {
    return Wrap(
      spacing: 5,
      children: ["Tell me a joke", "Give me a quote", "Help with commands"].map((suggestion) {
        return ActionChip(
  ...
  Widget _buildChatBubble(Map<String, String> message) {
    ...
    return Column(
      ...
      children: [
        ...
        if (!isUser && !message["text"]!.contains("Error")) // Show quick replies only for valid responses
          Padding(
